...
double complex screen_to_complex_plane_coordinates(
    ...
    int size) {
    double x = map(creal(coordinates),
                   0,
                   size,
                   creal(screen_center_in_complex_plane) - width / 2,
                   creal(screen_center_in_complex_plane) + width / 2);
    double y = map(cimag(coordinates),
                   0,
                   size,
...
double complex pixel_get_complex_plane_coordinates(Pixel* pixel) {
    ...
    pixel->_complex_plane_coordinates = screen_to_complex_plane_coordinates(
        ...
        pixel->_state->complex_width,
        pixel->_state->window->size);
...
Pixel pixel_add(Pixel* pixel1, Pixel* pixel2) {
    if (pixel1->_screen_coordinates_cached &&
        pixel2->_screen_coordinates_cached)
        return pixel_new_from_screen_coordinates(
            pixel1->_state,
            pixel1->_screen_coordinates + pixel2->_screen_coordinates);
        ...
        return pixel_new_from_complex_plane_coordinates(
            pixel1->_state,
            pixel1->_complex_plane_coordinates +
        ...
        return pixel_new_from_complex_plane_coordinates(
            pixel1->_state,
            pixel_get_complex_plane_coordinates(pixel1) +
                pixel2->_complex_plane_coordinates);
    }
...
Pixel pixel_add_value(Pixel* pixel,
    ...
    switch (type) {
        ...
        case COORDINATES_TYPE_COMPLEX_PLANE:
            return pixel_new_from_complex_plane_coordinates(
                pixel->_state,
                pixel_get_complex_plane_coordinates(pixel) + value);
            break;
    }
